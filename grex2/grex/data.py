...
def extract_data(treebank_paths, scope, conclusion, conclusion_meta, feature_predicate, include_metadata=False):
    ...
    for tp in treebank_paths:
        ...
        if conclusion is not None:
            tmp_matches = corpus.search(req, clustering_parameter=["{" + conclusion + "}"])
            matches = [(sent, c, draft[sent['sent_id']].meta['language']) for c, sents in tmp_matches.items() for sent in sents]

            languages = [
                draft[sent['sent_id']].meta['language']
                for _, sents in tmp_matches.items()
                for sent in sents
                if "language" in draft[sent['sent_id']].meta
            ]

        else:
            tmp_matches = [(sent, "Yes", sent['sent_id']) for sent in corpus.search(req)]
            languages = []
        ...
        if conclusion_meta is not None:
            ...
            matches = [
                ...
                if c == "No"
                    ...
                    (sent, "Yes", sent['sent_id'])
                    if all(
                        any(re.fullmatch(p, draft[sent['sent_id']].meta[k]) for p in v)
                        for k, v in conclusion_meta.items()
                    )
                    else (sent, "No", sent['sent_id'])
                )
                for sent, c, _ in tmp_matches
    ...
    return sanitized_data, languages
